...
describe('http utils', () => {
  ...
  describe('apiRequest', () => {
    ...
    test('sends GET request with Authorization header', async () => {
      ...
      expect(
        ((options as Record<string, unknown>).headers as Record<string, string>)['Authorization']
    ...
    test('exits process on TOKEN_INVALID error (api key not found or invalid)', async () => {
      mockFetch.mockResolvedValueOnce({
        status: 401,
        json: async () => ({
          lang_id: 1033,
          status_code: 'k_ec_000015',
          error_msg: 'Invalid Request: api key not found or invalid',
        }),
      } as unknown as ReturnType<typeof fetch>);
    ...
    test('exits process on TOKEN_MISSING error (missing or invalid Authorization header)', async () => {
      mockFetch.mockResolvedValueOnce({
        status: 401,
        json: async () => ({
          lang_id: 1033,
          status_code: 'k_ec_000015',
          error_msg: 'Invalid Request: missing or invalid Authorization header',
        }),
      } as unknown as ReturnType<typeof fetch>);
      ...
      await expect(
        apiRequest('GET', '/openapi/dau', { token: '' })
      ).rejects.toThrow('process.exit(1)');
