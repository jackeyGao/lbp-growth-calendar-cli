...
export function registerPenetrationCommand(program: Command): void {
  ...
  // upsert - 更新或插入（幂等，唯一接口）
  penetration
    .command('upsert')
    .description('按幂等键更新或插入 AI 渗透数据（free: dataDate+type+tenantType+aiPaymentStatus; paid: dataDate+type+customerIndustry+aiPaymentStatus）')
    .requiredOption('--type <type>', '类型: free | paid')
    .requiredOption('--date <date>', '数据日期，YYYY-MM-DD')
    .requiredOption('--ai-payment-status <status>', 'AI付费状态: paid | unpaid（作为幂等键之一）')
    ...
    .action(async (opts) => {
      try {
        ...
        const body: Record<string, unknown> = {
          type: opts.type,
          dataDate: opts.date,
          aiPaymentStatus: opts.aiPaymentStatus,
        };
