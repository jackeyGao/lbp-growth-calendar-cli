...
export function registerAuthCommand(program: Command): void {
  ...
  auth
    ...
    .action(async (code: string | undefined, opts: { apiKey?: string }) => {
      try {
        ...
        if (hasCode && hasApiKeyOption) {
          outputError(
            `不能同时提供 code 和 --api-key 参数，请选择其中一种方式。`,
            'INVALID_PARAMS'
          );
          return;
        }
        ...
        if (!hasCode && !hasApiKeyOption) {
          outputError(
            `需要提供 code 或 --api-key 参数之一。用法：\n1. auth verify <code> - 用授权码查询授权状态\n2. auth verify --api-key <key> - 验证 API Key 是否有效`,
            'MISSING_PARAMS'
          );
          return;
        }
        ...
        if (hasApiKeyOption) {
          ...
          const { status, data } = await apiRequestWithApiKey(
            'POST',
            '/openapi/agent-auth/verify',
            apiKey
          );
          ...
          if (status === 400) {
            ...
            outputError(
              `请求参数错误: ${response.message || '请检查 API Key 格式'}`,
              'BAD_REQUEST'
            );
            return;
          }
